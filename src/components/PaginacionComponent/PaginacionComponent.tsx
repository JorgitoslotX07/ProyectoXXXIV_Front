...
export const PaginacionComponent: FC<PaginacionProps> = ({
  ...
  <div className="flex items-center space-x-2">
    ...
    <select
      ...
      value={pageSize}
      onChange={(e) => onPageSizeChange(Number(e.target.value))}
      className="px-2 py-1 border border-gray-600 rounded-md bg-[#1F2937] text-gray-300 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500"
    >
      {pageSizes.map((size) => (
        <option key={size} value={size} className="bg-[#1F2937] text-gray-300">
          {size}
        </option>
      ))}
    </select>
