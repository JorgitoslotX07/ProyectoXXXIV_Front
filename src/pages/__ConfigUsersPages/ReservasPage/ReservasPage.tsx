...
export const ReservasPage: FC = (): ReactElement => {
    ...
    useEffect(() => {
        const fetchReservas = async () => {
            try {
                const data = await httpGetTok<ReservaDetalle[]>(`/reservas/usuario/${usuario.username}`);
                                ...
                                >
                                    <div className="p-4  space-y-2">
                                        <div className="flex justify-between items-center">
                                            ...
                                            <p className={`text-sm font-medium ${colorPorEstado(reserva.estado)}`}>
                                        ...
                                        <div className="grid grid-cols-2 md:grid-cols-3 gap-2 text-sm text-gray-600">
                                            <p>
                                                <span className="font-medium text-gray-500">Fecha Reserva:</span>{" "}
                                                {new Date(reserva.fechaReserva).toLocaleDateString()}
                                            </p>
                                            <p>
                                                <span className="font-medium text-gray-500">Inicio Viaje:</span>{" "}
                                                {new Date(reserva.fechaInicioViaje).toLocaleDateString()}
                                            </p>
                                            {reserva.fechaFinViaje && (
                                                <p>
                                                    <span className="font-medium text-gray-500">Fin Viaje:</span>{" "}
                                                    {new Date(reserva.fechaFinViaje).toLocaleDateString()}
                                                </p>
                                            )}
                                            {reserva.kmRecorridosViaje !== null && (
                                                <p>
                                                    <span className="font-medium text-gray-500">Km Recorridos:</span>{" "}
                                                    {reserva.kmRecorridosViaje}
                                                </p>
                                            )}
                                            <p>
                                                <span className="font-medium text-gray-500">Parking Recogida:</span>{" "}
                                                {reserva.parkingRecogidaNombre}
                                            </p>
                                            <p>
                                                <span className="font-medium text-gray-500">Parking Devolución:</span>{" "}
